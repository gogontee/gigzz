...
export default function ContractJobs() {
  ...
  // Fetch Contract Jobs
  async function fetchContractJobs(initial = false) {
    try {
      setLoading(true);
      ...
      console.error("Error fetching contract jobs:", err.message);
    } finally {
      setLoading(false);
    }
  }
  ...
  if (loading && jobs.length === 0) {
    return <p className="text-center py-10">Loading Contract Jobs...</p>;
  }

  if (!loading && jobs.length === 0) {
    return <p className="text-center py-10">No contract jobs available.</p>;
  }
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-xl md:text-2xl font-bold">Contract Jobs</h1>
