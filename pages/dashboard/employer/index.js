...
export default function EmployerDashboard() {
  ...
  // 🛡️ Check if the current user is an admin
  useEffect(() => {
    if (!user?.id) return;
      ...
      <div className="flex-1 overflow-auto">
        ...
        <div className="flex items-center justify-between px-2 lg:px-6 py-4 lg:pt-20 border-b border-gray-200">
          ...
          <div className="flex gap-4 items-center">
            ...
            {/* 🛡️ Admin panel shortcut — only for admins */}
            {isAdmin && (
              <button
                onClick={() => router.push('/admins')}
                title="Admin Panel"
                aria-label="Admin Panel"
                className="relative p-1.5 rounded-lg hover:bg-orange-50 transition-colors group"
              >
                <ShieldCheck className="w-6 h-6 text-black group-hover:text-orange-600 transition-colors" />
              </button>
            )}
